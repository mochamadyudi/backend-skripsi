...
export default class UserService extends LibService{
    constructor(props) {
        super(props);
        this.schema = _MUser.Provider
    }
    ...
    async list(){
        try{
            ...
            let condition = {_id: {$ne: this.req?.user.id}}

            if(ObjResolve(this.query,'role')){
                Reflect.set(condition,'role', {
                    $in: Array.isArray(this.query.role) ? this.query.role : [ this.query.role ]
                })
            }
